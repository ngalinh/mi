...
**Khai báo tài khoản/mật khẩu:**

- **Dùng chung 1 tài khoản cho mọi profile** (trường hợp phổ biến — tất cả profile Zalo của nhân
  viên đăng nhập qua **cùng 1 tài khoản ZaloCRM**): chỉ cần đặt **1 lần** trong `.env` của runner,
  áp dụng cho **mọi** profile:
  ```
  SALEWORK_LOGIN_USER=...      # email đăng nhập zalo.basso.vn
  SALEWORK_LOGIN_PASS=...      # mật khẩu
  ```
- **Theo từng nhân viên** (chỉ khi có profile dùng tài khoản KHÁC — ưu tiên hơn `.env`): dashboard
  → *Cài đặt → Tài khoản* → mở account Zalo, điền **Tài khoản/Mật khẩu đăng nhập Zalo Basso**. Mật
  khẩu **không bao giờ** trả ngược ra API (ẩn khỏi `/api/accounts`); khi sửa để trống ô mật khẩu =
...
1. **Poller định kỳ** — server tự quét mỗi `AUTO_NOTIFY_INTERVAL_MS` (mặc định 1 phút).
   Bật bằng `.env`:
   ```
   AUTO_NOTIFY=true
...
2. **Webhook real-time** — để website Basso gọi sang **ngay khi có hàng về**:
   ...
   Lượt webhook **đọc tươi từ Basso** (bỏ qua cache 30s) để thấy NGAY đơn vừa về. Handler
   **không đọc body** — chỉ cần Basso "ping"; secret so khớp constant-time, miễn trừ gateway
   secret. Không phụ thuộc `AUTO_NOTIFY` bật/tắt (vẫn tôn trọng `autoEnabled` từng NV).
...
- **Không gửi chồng tay ↔ tự động**: hai luồng dùng chung 1 khóa (`server/lock.js`) nên chạy
  ...
  chỉ nhận đúng 1 tin.)
- **Bật auto = chỉ báo đơn về TỪ ĐÓ trở đi** (`AUTO_NOTIFY_ONLY_NEW=true`, mặc định): khi 1
  nhân viên bật "Tự động báo" cho tài khoản của mình, hệ thống đóng dấu mốc `autoEnabledAt`.
  Bot **bỏ qua đơn tồn đọng về TRƯỚC ngày bật** (so theo `date_inventory`) → tránh nhắn trùng
  loạt khách cũ đã xử lý tay trong lúc auto còn tắt. Đơn về từ ngày bật trở đi mới được tự
  gửi. Muốn bot gửi cả tồn đọng (hành vi cũ) thì đặt `AUTO_NOTIFY_ONLY_NEW=false`. Tài khoản
  đã bật auto từ trước (chưa có mốc) giữ nguyên hành vi cũ tới lần bật/tắt kế tiếp.
- **Cô lập theo nhân viên** (`AUTO_NOTIFY_REQUIRE_ACCOUNT=true`): bot **chỉ tự gửi đơn khớp
  đúng 1 tài khoản Zalo đang bật auto** (`staffId`/tên khớp NV); đơn không khớp account nào
  (rơi về mặc định) hoặc account tắt auto đều bỏ qua. Tiện để **test 1 NV**: bật auto cho 1
  account là chỉ NV đó được gửi, khỏi phải đi tắt từng account chưa map. Mặc định `false` (đơn
  không khớp vẫn gửi bằng account mặc định như cũ).
...
Khi chạy có cửa sổ (`HEADLESS=false`), browser mở phóng to và vùng trang tự co theo kích thước cửa sổ thực tế, kể cả khi đổi kích thước Remote Desktop. Không ép viewport 1366 × 850 làm cắt phần bên phải hoặc ô nhập chat phía dưới. Chế độ headless vẫn giữ kích thước 1366 × 850. Sau khi cập nhật cần khởi động lại local-runner để tạo cửa sổ với cấu hình mới.
