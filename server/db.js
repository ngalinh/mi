...
// Migration: KÊNH gửi — 'facebook' | 'zalo'. Để Lịch sử báo hiện icon kênh trước tài khoản.
...
// Kiểu báo RIÊNG cho khách này, ghi đè "Kiểu báo" của NV phụ trách (trường hợp ngoại lệ): NULL/'' =
...
function normReportTarget(v) {
  ...
  return s === 'personal' || s === 'group' ? s : null;
}

/**
 * Kiểu báo RIÊNG (ghi đè) đã lưu cho 1 SĐT khách: 'personal' | 'group' | '' (rỗng = theo NV).
 * Dùng khi gửi để ép đúng tab hội thoại (cá nhân/nhóm) cho khách ngoại lệ.
 */
function getContactReportTarget(phone) {
  const p = normPhone(phone);
  if (!p) return '';
  const r = getZaloContactStmt.get({ phone: p });
  ...
  try {
    ...
    for (const row of list) {
      ...
      upsertZaloContactStmt.run({
        ...
        source: 'import',
        // Import chỉ cập nhật tên/ghi chú — giữ nguyên cấu hình báo qua FB / kiểu báo đã có của số này.
