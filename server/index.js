...
// Tạo file danh sách tài khoản nếu chưa có (seed từ .env nếu có BASSO_EMAIL).
auth.ensureAccountsFile();
...
// ---- Đăng nhập / đăng xuất / thông tin người dùng ----
app.post('/api/auth/login', async (req, res) => {
  try {
    const { email, pass } = req.body || {};
    const { sid, user } = await auth.login(email, pass);
...
app.get('/api/auth/me', (req, res) => {
  ...
  if (!s) return res.status(401).json({ ok: false, auth: false, error: 'Chưa đăng nhập' });
  res.json({ ok: true, user: { email: s.email, name: s.name } });
});

// ---- Webhook: website Basso gọi sang khi CÓ HÀNG VỀ -> gửi ngay (real-time) ----
// Máy-gọi-máy nên KHÔNG bắt đăng nhập; bảo vệ tùy chọn bằng header `x-webhook-secret`
// khớp AUTO_NOTIFY_WEBHOOK_SECRET. Tác vụ này chạy dưới tài khoản hệ thống (.env).
...
// ---- Asset tĩnh dùng chung (css, js, font) + trang đăng nhập: công khai ----
// Tắt cache asset tĩnh (dev): tránh trình duyệt giữ JS/CSS cũ sau khi sửa code.
app.use('/css', express.static(path.join(PUBLIC_DIR, 'css'), { etag: false, lastModified: false, setHeaders: noCache }));
app.use('/js', express.static(path.join(PUBLIC_DIR, 'js'), { etag: false, lastModified: false, setHeaders: noCache }));
...
// ============================================================
//  GÁC CỬA — mọi thứ phía dưới cần đăng nhập
// ============================================================
app.use(auth.requireAuth);

// Trang HTML (dashboard, reports) + root: phải đăng nhập mới xem được.
app.use(express.static(PUBLIC_DIR, { etag: false, lastModified: false, setHeaders: noCache }));
...
app.get('/api/basso/ping', async (req, res) => {
  ...
  const t0 = Date.now();
  try {
    const r = await req.basso.getOrders({ pageSize: 1 });
...
app.get('/api/orders', async (req, res) => {
  try {
    ...
    const data = await req.basso.getOrders({ from, to, status, staff, q });
...
app.get('/api/arrived-items', async (req, res) => {
  try {
    ...
    const data = await req.basso.getArrivedItems({ id, customerId, dateInventory });
    res.json({ ok: true, ...data });
  } catch (err) {
    res.status(500).json({ ok: false, error: err.message });
  }
...
app.post('/api/notify', async (req, res) => {
  try {
    ...
    // Cập nhật trạng thái ngược về web chạy dưới tài khoản Basso của người đang đăng nhập.
    const opts = { profile, account, messageOverride, kind, bassoClient: req.basso };
...
app.post('/api/update-row', async (req, res) => {
  try {
    ...
    const result = await req.basso.updateOrderStatus({ customerId, dateInventory, status, note });
    res.json({ ok: true, ...result });
  } catch (err) {
    res.status(500).json({ ok: false, error: err.message });
  }
...
app.listen(config.port, () => {
  ...
  console.log(`[server] tài khoản đăng nhập: ${auth.loadAccounts().length} (sửa ${config.auth.accountsPath} để thêm/xóa)`);
