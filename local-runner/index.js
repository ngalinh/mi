 ...
 *         sharedStaffIds?, brand?, kenhSale?, autoEnabled?, proxy?, notifyTarget? }
 */
app.post('/api/accounts', (req, res) => {
  const { type, platform: platformIn, key, name, saleworkName, fbName, email, password, phone, staffId, sharedStaffIds, brand, kenhSale, autoEnabled, proxy, notifyTarget } = req.body || {};
  const platform = (platformIn || type) === 'facebook' ? 'facebook' : 'zalo';
  const label = platform === 'facebook' ? 'Facebook' : 'Zalo';
  let account;
  try {
    account = accountsStore.add({ platform, key, name, saleworkName, fbName, email, password, phone, staffId, sharedStaffIds, brand, kenhSale, autoEnabled, proxy, notifyTarget });
  } catch (e) {
...
app.put('/api/accounts/:key', (req, res) => {
  ...
  const { name, saleworkName, fbName, email, password, phone, staffId, sharedStaffIds, brand, kenhSale, autoEnabled, proxy, notifyTarget } = req.body || {};
  const patch = {};
  for (const [k, v] of Object.entries({ name, saleworkName, fbName, email, password, phone, staffId, sharedStaffIds, brand, kenhSale, autoEnabled, proxy, notifyTarget })) {
